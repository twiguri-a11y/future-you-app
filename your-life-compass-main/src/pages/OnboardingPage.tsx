...
export const ONBOARDING_KEY = "fy_onboarding";
export const ONBOARDING_COMPLETE_KEY = "fy_onboarding_complete";
...
export function onboardingHash(data: OnboardingData): string {
  return [
    ...data.selectedIdentities.sort(),
    data.customIdentity.trim(),
    data.priority ?? "",
...
const OnboardingPage = () => {
  ...
  const canSubmit =
    (form.selectedIdentities.length >= 1 || form.customIdentity.trim().length > 0) &&
    form.priority !== null;
  ...
  const handleSubmit = () => {
    ...
    localStorage.setItem("fy_onboarding_hash", onboardingHash(form));
    navigate("/vision-moment", { state: { visionSummary } });
  };

  return (
    <div className="onboarding-root fixed inset-0 overflow-y-auto" style={{ zIndex: 50 }}>
      <div className="min-h-full flex items-start justify-center px-4 py-8 md:py-12">
        <div className="w-full max-w-[640px] space-y-6">
          {/* Header */}
          <div className="text-center space-y-2">
            <div className="mx-auto w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "rgba(196,138,90,0.15)" }}>
              <Sparkles className="w-5 h-5" style={{ color: "#C48A5A" }} />
            </div>
            <h1 className="onboarding-text-primary font-display text-2xl md:text-3xl font-semibold" style={{ letterSpacing: "-0.01em" }}>
              ...
              {identityOptions.map((id, i) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`rounded-xl px-3 py-2 text-sm font-body font-medium transition-all duration-200 text-center ${isSelected ? "onboarding-chip-selected" : "onboarding-chip"}`}
                  >
          ...
          {/* Section 2: Priority */}
          <Section title="What matters most right now?" subtitle="Pick your current focus">
            <div className="grid grid-cols-2 gap-2">
              {priorities.map((p) => (
                <motion.button
                  key={p}
                  whileTap={{ scale: 0.97 }}
                  onClick={() => setForm((prev) => ({ ...prev, priority: prev.priority === p ? null : p }))}
                  className={`rounded-xl px-3 py-2.5 text-sm font-body font-medium transition-all duration-200 text-center ${form.priority === p ? "onboarding-chip-selected" : "onboarding-chip"}`}
                >
                  {p}
                </motion.button>
              ))}
          ...
          <Section title="Your environment" subtitle="Where do you spend most of your time?">
            <input
          ...
          {/* Section 4: People */}
          <Section title="People around you" subtitle="Who's part of your daily life?">
            <input
              value={form.people}
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {emotions.map((e) => (
                <motion.button
                  key={e}
                  whileTap={{ scale: 0.97 }}
                  onClick={() => setForm((prev) => ({ ...prev, emotion: prev.emotion === e ? null : e }))}
                  className={`rounded-xl px-3 py-2 text-sm font-body font-medium transition-all duration-200 text-center ${form.emotion === e ? "onboarding-chip-selected" : "onboarding-chip"}`}
                >
                  {e}
                </motion.button>
              ))}
          ...
          <motion.button
            ...
            style={{
              ...
              boxShadow: canSubmit ? "0 4px 16px rgba(31,31,31,0.2)" : "none",
            }}
          >
            Create my future
...
function Section({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <div className="onboarding-card rounded-2xl p-5 space-y-3">
      <div>
        <h2 className="onboarding-text-primary font-display text-base md:text-lg font-semibold" style={{ letterSpacing: "-0.01em" }}>
          {title}
        </h2>
        {subtitle && <p className="onboarding-text-secondary text-xs font-body mt-0.5">{subtitle}</p>}
      </div>
      {children}
    </div>
  );
